...
import { AsyncCap, newCid, partition, printLn, shuffle, sum, trackProgress, waitAll, } from './util';
...
async function groupFiles(files) {
    ...
    let progress = new Progress(sum(todo, group => sum(group, file => file.size)));
...
FileStream.OpenFilesCounter = new AsyncCap(MAX_OPEN_FILES);
